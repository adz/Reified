...
Status: partly done. Sections 1–4 and 7 are complete — this repository is the extracted description side,
renamed to `Reified.*`, with the umbrella package added and Flow left behind in Axial. Sections 5 and 6, the
per-project documentation plan and FsLiveDocs, are the outstanding work.

Package names below are as they stood at the split. `Reified.Refined` shipped as `Reified.Refinements`, and the
two host HTTP adapters left with Axial rather than being renamed here.

Everything else below is work to do. Reasoning that led here is not repeated.
