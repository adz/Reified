...
- **No model-binding integration by default.** Reified validates values and models you hand it; wiring that
  into an ASP.NET pipeline is [`Reified.Schema.Http`](/http-contracts/index.html)'s job, not an
...
- [`Reified.Result` against FsToolkit.ErrorHandling](/how-it-compares/fstoolkit-comparison.html) — for
...
- Decoding serialized input is a different job from validating a typed value: see
  [`Reified.Parse`](/parsing-input/index.html) and [Schema](/modelling/index.html) rather than this page.
