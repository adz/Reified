...
The `result { }` above is [`Reified.Result`](/validating-values/result/index.html) and is optional — these packages return the
standard F# `Result`, so any Result vocabulary composes them.

Start with [Getting Started](/getting-started/index.html), then read [Define Refined Types](/domain-types/domain-values.html).

For compact prompt context, load [`/validating-values/llms.txt`](/validating-values/llms.txt).
