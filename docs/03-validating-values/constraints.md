...
are also, by default, untranslatable — see [Custom rules](/validating-values/localization/custom-rules.html) for `customLocalized`, which lets you
...
Localization applies to failures, and is covered [there](/validating-values/localization/index.html).
...
`Constraint.present` and `Constraint.trimmed` are weakened rather than exact because [blankness](/validating-values/overview.html#what-blank-means)
