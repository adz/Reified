...
Look keys up in [the catalogue](/validating-values/localization/catalogue.html).
...
and check what is missing with [Adding a language](/validating-values/adding-a-language.html).

### Translate one of your own rules

Give the rule a key and it joins the same lookup — see [Custom rules](/validating-values/localization/custom-rules.html).
...
- [Context and fallback](/validating-values/localization/context-and-fallback.html) — scoping a renderer to a document and a field, and how lookup
  falls back.
- [Custom rules](/validating-values/localization/custom-rules.html) — making your own constraints translatable.
- [Advanced rendering](/validating-values/localization/advanced-rendering.html) — composition, interpolation, groups, plurals, and resolvers.
- [The key catalogue](/validating-values/localization/catalogue.html) — every key Reified can produce.
- [Adding a language](/validating-values/adding-a-language.html) — the working order for a new translation, and proving coverage.
- [Fable support](/validating-values/fable.html) — what the rendering edge does and does not do in JavaScript.

Schema renders its own parse and structural failures through these same mechanics, with the field path supplied for
you: see [Redisplay and field errors](/modelling/redisplay-and-field-errors.html).
