---
title: For AI agents
description: High-signal Reified.Result guidance for coding agents.
weight: 100
...
`Reified.Result` is a standalone leaf: it depends on no other Reified package, and nothing in Reified depends on it. It works
...
- Accumulation here is flat: a container of error values with no path and no field identity. Path-aware accumulation
  is `Reified.Schema`'s, and is not interchangeable with this.
...
Start with [Result](./), then read [Collecting every error](/validating-values/result/collecting-errors.html).

For compact prompt context, load [`/validating-values/result/llms.txt`](/validating-values/result/llms.txt).
