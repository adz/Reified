...
[When not to make a type](/domain-types/catalog.html#when-not-to-make-a-type).

`Reified.Refinements` depends only on `Reified.Constraint`. It does not
[parse text](/parsing-input/index.html) and does not normalize input.
...
[the catalogue](/domain-types/catalog.html#everyday-operations).

[Order Totals](/domain-types/tutorials/order-totals.html) works this through on a realistic domain.
...
[the catalogue](/domain-types/catalog.html#floating-point).
...
[Customer Id](/domain-types/tutorials/customer-id.html) builds one end to end, including its schema.
...
other field's. See [Schema integration](/modelling/index.html).

## Read next

1. [Order Totals](/domain-types/tutorials/order-totals.html) uses the built-in types in anger.
2. [Built-in Refined Values](/domain-types/catalog.html) lists what each type buys you.
3. [Customer Id](/domain-types/tutorials/customer-id.html) defines a refined type of your own.
4. [Define Refined Types](/domain-types/domain-values.html) is the reference for `Refinement`.
5. [Schema Integration](/modelling/index.html) applies refinements at structured boundaries.
6. [Compose Parse and Refinement](/domain-types/composition.html) maps failures into application errors.
