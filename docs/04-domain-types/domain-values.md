...
[constraint](/validating-values/constraint.html) on the primitive instead. Numeric ranges are the clearest
example — F# cannot carry "greater than zero" through arithmetic, so a refined number costs more at every use site
than it saves. [When not to make a type](/domain-types/catalog.html#when-not-to-make-a-type) draws the line, and
[Customer Id](/domain-types/tutorials/customer-id.html) works a full example through.
...
[interpreted](/validating-values/constraints.html): Reified can read the rule as data, so the refinement
...
[interpreted versus opaque](/validating-values/constraints.html):
...
Continue with [Schema Integration](/modelling/index.html).
