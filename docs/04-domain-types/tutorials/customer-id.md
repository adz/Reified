...
| `NormalisedName` — trimmed and lower-cased | **Neither.** That is a transformation, so it belongs in [Parse](/parsing-input/index.html). |
...
[Define Refined Types](/domain-types/domain-values.html) for both.
...
in [Refined Schemas](/modelling/refined-values.html), which works the same
example through the Schema DSL and shows where schema-local constraints fit alongside it.

## Next

- [Order Totals](/domain-types/tutorials/order-totals.html) — the built-in types used in anger.
- [Define Refined Types](/domain-types/domain-values.html) — the full `Refinement` reference.
- [Schema Integration](/modelling/index.html) — applying refinements at structured boundaries.
- [Refined Schemas](/modelling/refined-values.html) — the Schema-side view.
