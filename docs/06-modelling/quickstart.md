...
[SchemaDSL](dsl.html) and [Compiler-Directed, AOT, and Fable](/notes/aot-trimming-fable.html).

### Parse

`Data` is a source-neutral input tree. The same schema reads form posts, CLI arguments, JSON, and configuration —
see [Input Sources](/modelling/input-sources.html).
...
[Nested Models And Collections](/modelling/tutorials/nested-and-collections.html).
...
path. See [JSON Codec](/json/index.html) for buffers, streams, and decode diagnostics.
...
see [HTTP Servers](/http-contracts/index.html).
...
[Reified.Refinements](/domain-types/index.html) ships the common ones. Use them as field types directly:
...
Your own domain types participate the same way. See [Refined Schemas](/modelling/refined-values.html) for defining `Email` or
...
[Constraints](/validating-values/constraints.html); see
[Refined Schemas](/modelling/refined-values.html) for application-defined constraints.
...
- [Construction Guarantees](/modelling/trusted-construction.html) — what each rung does and does not promise.
...
- [Field Blocks and Plain Functions](/modelling/field-desugaring.html) — a field block read as ordinary functions over one `Schema`.
- [Refined Schemas](/modelling/refined-values.html) — your own domain types as fields.
...
- [Versioned Contracts](/http-contracts/contracts.html) — evolving the wire format without freezing the domain model.
