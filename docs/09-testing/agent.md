---
title: For AI agents
description: High-signal Reified.Data guidance for coding agents.
weight: 100
...
Use `Reified.DataDSL` for source-neutral fixtures and produced output. Construct values with `data`, derive related
...
`JSON.parse` result. Install `Reified.Schema.Json` and use `Json.parseData` when portable, lossless JSON parsing is needed.

For compact prompt context, load [`/testing/llms.txt`](/testing/llms.txt).
