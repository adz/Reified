...
Reified is pre-1.0 and has not been published to NuGet yet, so these `dotnet add package` lines do not resolve for now.
...
See [Derived Schemas](/modelling/derivation/index.html) and [Testing patterns](/testing/testing-schema-guarantees.html).
