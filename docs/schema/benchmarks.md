...
The boundary suite compares the trusted codec lane against full boundary parsing — `JsonDocument` to `RawInput` to `Schema.parse` with complete path-aware diagnostics:

| Method | Mean | Allocated |
| --- | --- | --- |
| `Axial Json.deserialize (trusted path)` | 3.15 us | 2.84 KB |
| `JsonDocument + RawInput + Schema.parse (boundary lane)` | 19.78 us | 27.71 KB |
...
- `BoundaryParseBenchmarks`: trusted codec lane vs `RawInput` + `Schema.parse` boundary lane
