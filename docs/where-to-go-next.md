...
| | |
| --- | --- |
| [Constraint](/values/constraint/) | Reusable rules that carry their own explanation, in any language |
| [Refined values](/values/refined/) | Types that can only hold values the rule admits — `NonBlankString`, your own `CustomerId` |
| [Parse](/values/parse/) | Serialized primitives to typed values, with the failure as data |
...
| | |
| --- | --- |
| [Schema](/schema/quickstart/) | One declaration per model, read by every layer that would otherwise repeat it |
| [What a schema gives you](/what-a-schema-gives-you/) | The seven jobs around a model that one declaration already does |
| [JSON codecs](/schema/json-codec/) | Reflection-free JSON, so decoding and validation cannot drift apart |
| [HTTP contracts](/schema/http-servers/) | Endpoints whose OpenAPI is generated from the same declaration they enforce |
| [Contracts](/schema/contracts/) | Old payload versions migrated into the current model, on purpose |
...
| | |
| --- | --- |
| [Result](/result/) | Fail-fast sequencing and error accumulation over your own error type |
| [Data](/data/) | Building, editing, and comparing test data without hand-writing every case |
...
[Packages and platforms](/notes/packages-and-platforms/) for the list, what each one gives you, and which run
