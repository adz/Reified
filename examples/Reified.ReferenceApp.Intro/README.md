# Introductory reference app (Result and the Values packages only)

A conference registration desk built with only `Reified.Result`, `Reified.Constraint`, `Reified.Refinements`, and
`Reified.Parse`. It is the first tier of the
reference apps: everything here is plain F# `Result` with your own error union, and no Reified type appears in a
domain signature unless you choose a refined value.

The full reference app in [`examples/Reified.ReferenceApp`](../Reified.ReferenceApp/) is the next tier: the same
