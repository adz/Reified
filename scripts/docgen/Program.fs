...
let pageSpecs = [
    ...
    }
    {
        ...
        Intro = "This page shows the `Check` surface for reusable, path-free value constraints. Executable checks return `Result<unit, CheckFailure list>` and can be composed with `Check.all`, `Check.any`, `Check.not`, and `Check.mapFailure`. Top-level helpers such as `Check.notBlank` remain lightweight boolean predicates for local structural facts. Use `Result.require` or `Result.guard` when a `Check<'value>` should become a fail-fast result that preserves the checked value."
        SymbolIds = [
            ...
            "Executable number checks", ["M:Axial.ErrorHandling.CheckModule.Number.between"; "M:Axial.ErrorHandling.CheckModule.Number.greaterThan"; "M:Axial.ErrorHandling.CheckModule.Number.lessThan"; "M:Axial.ErrorHandling.CheckModule.Number.atLeast"; "M:Axial.ErrorHandling.CheckModule.Number.atMost"]
            "Executable collection checks", ["M:Axial.ErrorHandling.CheckModule.Collection.notEmpty"; "M:Axial.ErrorHandling.CheckModule.Collection.minCount"; "M:Axial.ErrorHandling.CheckModule.Collection.maxCount"; "M:Axial.ErrorHandling.CheckModule.Collection.countBetween"; "M:Axial.ErrorHandling.CheckModule.Collection.distinct"]
