...
type RefinementError =
    ...
    /// <summary>An executable <see cref="T:Axial.Check.Check`1" /> program run against the target refined type
    /// failed. Carries the same structured <see cref="T:Axial.Check.CheckFailure" /> values the check program
    ...
    /// <see cref="T:Axial.Check.Check`1" /> program cannot express, such as a cross-field ordering
    /// invariant.</summary>
...
module RefinementError =
    /// <summary>Renders a single refinement error using the supplied <see cref="T:Axial.Check.CheckFailureResources" />
    ...
    /// <see cref="P:Axial.Check.CheckFailure.english" /> for its <c>CheckFailed</c> failures.</summary>
