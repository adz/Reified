...
/// manager wrapper all satisfy it. Reified owns the candidate order, so a lookup only ever answers "do you have this
/// exact key".
/// </remarks>
...
type MessageResolution =
    /// <summary>A template Reified should interpolate and format.</summary>
    | Template of string
    /// <summary>Text the resolver has already rendered. Reified never interpolates it again.</summary>
    | Rendered of string

/// <summary>Resolves one contextual level, or declines so Reified continues to a less specific one.</summary>
...
module Renderer =
    ...
    /// <summary>Replacing lookup, formatting, or inspecting the exact keys Reified will ask for.</summary>
    [<RequireQualifiedAccess>]
    module Advanced =
        /// <summary>A renderer backed by a resolver that answers one contextual level at a time.</summary>
        /// <remarks>
        /// Use this for ICU or any system that selects plural categories and renders entries itself. Reified still
        ...
        /// to test translation coverage for contexts and fields Reified cannot enumerate for you.
        /// </remarks>
        ...
        /// and adds no Schema knowledge to <c>Reified.Constraint</c>.
        /// </remarks>
