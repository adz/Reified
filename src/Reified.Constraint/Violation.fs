...
/// <remarks>
/// <para>
/// An interpreted constructor reports <c>Expected</c> carrying the very same <see cref="T:Reified.Constraint.ConstraintAtom" />
...
type AtomicViolation =
    ...
    /// it when a message tree is projected. Reified never invents one — a key it made up would promise a lookup
...
/// </para>
/// <para>
/// Reified-produced groups are never empty and never unary: a single failing child is returned directly rather than
...
/// own catalogue key. Reified never invents one: a key it made up would promise a lookup that cannot exist.
/// </remarks>
...
module Violation =
    ...
    /// </summary>
    /// <remarks>
    /// This is the normalization Reified itself applies, so an interpreter that accumulates failures from several
    /// constraints produces exactly the tree a single composed constraint would have produced. Reified-produced
