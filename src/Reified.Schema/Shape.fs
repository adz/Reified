...
/// <c>static member Schema: T -&gt; Schema&lt;T&gt;</c>; Reified supplies that member for its supported built-in types.
...
type SchemaDefaults =
    ...
    static member Schema(_: Reified.Refinements.NonBlankString) : Schema<Reified.Refinements.NonBlankString> =
        SchemaCore.refine Reified.Refinements.NonBlankString.refinement SchemaCore.text
    static member Schema(_: Reified.Refinements.FiniteFloat) : Schema<Reified.Refinements.FiniteFloat> =
        SchemaCore.refine Reified.Refinements.FiniteFloat.refinement SchemaCore.``float``
    static member Schema(_: Reified.Refinements.UnitInterval) : Schema<Reified.Refinements.UnitInterval> =
        SchemaCore.refine Reified.Refinements.UnitInterval.refinement SchemaCore.``float``
    ...
    static member NonEmptyArrayWith(item: Schema<'item>) : Schema<Reified.Refinements.NonEmptyArray<'item>> =
        // The wire shape is a list; the refinement bridges list -> NonEmptyArray while retaining minLength 1.
        SchemaCore.refine (Reified.Refinements.NonEmptyArray.listRefinement<'item> ()) (SchemaCore.listWith item)

    static member inline Schema(_: Reified.Refinements.NonEmptyArray< ^item>) : Schema<Reified.Refinements.NonEmptyArray< ^item>> =
        let inline resolve (witness: ^w, marker: ^value) : Schema< ^value> =
            ((^w or ^value): (static member Schema: ^value -> Schema< ^value>) marker)
