...
module internal ValueSchema =
    ...
    /// should use <see cref="M:Reified.Schema.Schema.nested``1" /> because their schema is already available.
    /// </remarks>
    ...
    /// <see cref="M:Reified.Schema.Schema.underlyingPrimitiveKind``1" /> to see through refinement layers to the primitive
    ...
    /// combined with <see cref="M:Reified.Schema.Schema.withConstraint``1" /> and attached to a field with
    /// <c>withSchema</c> like any other <see cref="T:Reified.Schema.Schema`1" />.
    /// </para>
    /// <para>
    /// The everyday raw schema is a primitive value schema, especially <see cref="P:Reified.Schema.Schema.text" /> for
    /// domain values such as email addresses and names. The primitive foundation stays inspectable through the
    /// refinement: <see cref="M:Reified.Schema.Schema.underlyingPrimitiveKind``1" /> reports the intrinsic primitive kind
    /// beneath any number of refinement layers, and <see cref="M:Reified.Schema.Schema.rawConstraints``1" /> returns the
    ...
    /// <see cref="M:Reified.Schema.Schema.withFormat``1" /> stays visible the same way:
    /// <see cref="M:Reified.Schema.Schema.format``1" /> reports the nearest declared format through refinement layers.
    /// </para>
    /// </remarks>
    ...
    /// <summary>Describes a union using one complete, inspectable wire representation.</summary>
    let unionWith representation (cases: UnionCase<'union> list) : Schema<'union> =
        ...
        let fieldModel (case: UnionCase<'union>) =
            match case.Definition.Payload with
            | FieldsUnionCase { Shape = NestedValueDefinition(model, _) } -> Some model
            | FieldsUnionCase _ ->
                invalidArg (nameof cases) $"Union case \"{case.Definition.Tag}\" fields must use a record schema."
            | _ -> None

        match representation with
        | UnionRepresentation.Internal tagField ->
            let tag = ExternalFieldName.create tagField |> ExternalFieldName.value
            for case in cases do
                match case.Definition.Payload with
                | EmptyUnionCase -> ()
                | ValueUnionCase _ -> invalidArg (nameof cases) $"Internal union case \"{case.Definition.Tag}\" must use UnionCase.fields."
                | FieldsUnionCase _ ->
                    let model = fieldModel case |> Option.get
                    if model.Fields |> List.exists (fun field -> ExternalFieldName.value field.ExternalName = tag) then
                        invalidArg (nameof cases) $"Union case \"{case.Definition.Tag}\" has a field that collides with discriminator \"{tag}\"."
        | UnionRepresentation.Adjacent(tagField, payloadField, style) ->
            let tag = ExternalFieldName.create tagField |> ExternalFieldName.value
            let payload = ExternalFieldName.create payloadField |> ExternalFieldName.value
            if tag = payload then invalidArg (nameof representation) "Adjacent union tag and payload fields must differ."
            if style = UnionPayloadStyle.UnwrappedSingle then
                for case in cases do
                    match fieldModel case with
                    | Some model when model.Fields.Length <> 1 -> invalidArg (nameof cases) $"Union case \"{case.Definition.Tag}\" must have exactly one field for UnwrappedSingle."
                    | _ -> ()
        | UnionRepresentation.External(style, _) when style = UnionPayloadStyle.UnwrappedSingle ->
            for case in cases do
                match fieldModel case with
                | Some model when model.Fields.Length <> 1 -> invalidArg (nameof cases) $"Union case \"{case.Definition.Tag}\" must have exactly one field for UnwrappedSingle."
                | _ -> ()
        | UnionRepresentation.External _ -> ()

        Schema(ValueDefinition
            { Shape = UnionValueDefinition { Representation = representation; Cases = cases |> List.map _.Definition }
              Format = None; Rules = []; Description = None; Default = None })

    /// <summary>Describes the recommended internally tagged union with discriminator <c>type</c>.</summary>
    let union cases = unionWith UnionRepresentations.recommended cases
    ...
    /// <see cref="M:Reified.Schema.Schema.withConstraint``1" />, which
    /// <see cref="M:Reified.Schema.Schema.constraints``1" /> continues to return.
    /// </remarks>
    /// <exception cref="T:System.ArgumentNullException">Thrown when <paramref name="schema" /> is null.</exception>
    ...
    /// <remarks>
    /// <para>
    /// Like <see cref="M:Reified.Schema.Schema.underlyingPrimitiveKind``1" />, this accessor is total over value schemas:
    ...
    /// <see cref="P:Reified.Schema.SchemaFormat.email" /> for a refined email address over
    /// <see cref="P:Reified.Schema.Schema.text" />. It is annotation metadata for interpreters — JSON Schema emitters,
    /// UI renderers, and documentation generators — and attaches no executable check; pair it with constraint
    /// metadata such as <see cref="P:Reified.Schema.Constraint.email" /> when validation is also required.
    /// </para>
    /// <para>
    ...
    /// <remarks>
    /// Like <see cref="M:Reified.Schema.Schema.underlyingPrimitiveKind``1" />, this accessor sees through refinement
    ...
    /// <remarks>
    /// Like <see cref="M:Reified.Schema.Schema.format``1" />, this accessor sees through refinement layers: a description
    ...
    /// <remarks>
    /// Like <see cref="M:Reified.Schema.Schema.format``1" />, this accessor sees through refinement layers: a default
    /// declared on the refined value schema itself wins, and otherwise the raw value schemas are walked toward the
    /// primitive foundation until a declaration is found.
    /// </remarks>
    /// <exception cref="T:System.ArgumentNullException">Thrown when <paramref name="schema" /> is null.</exception>
    ...
    /// result equals <see cref="M:Reified.Schema.Schema.constraints``1" />.
    /// </para>
    /// <para>
    ...
    /// <see cref="M:Reified.Schema.Schema.inspectUnderlying``2" /> — use this accessor so constraints declared on raw
    /// layers and on the refined schema are honored together. Per-layer inspection remains available through
    /// <see cref="M:Reified.Schema.Schema.constraints``1" /> and <see cref="M:Reified.Schema.Schema.rawConstraints``1" />.
    /// </para>
    /// </remarks>
    /// <exception cref="T:System.ArgumentNullException">Thrown when <paramref name="schema" /> is null.</exception>
