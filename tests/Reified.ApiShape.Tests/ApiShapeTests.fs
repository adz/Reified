...
module ApiShapeTests =
    ...
    let ``result builder remains while refinement builder is absent`` () =
        ...
        assertTypeAbsentFromAssembly "Reified.Refinements" "Reified.Refinements.RefineBuilder"
    ...
    let ``schema inspection and input interpreter modules expose the expected surface`` () =
        ...
        test <@ typeof<DataEdit>.Assembly.GetName().Name = "Reified.Data" @>
        test <@ typeof<DataPattern>.Assembly.GetName().Name = "Reified.Data" @>
    ...
    let ``removed validation surface is absent`` () =
        let assemblies =
            [ Assembly.Load "Reified.Result"
              Assembly.Load "Reified.Constraint"
              Assembly.Load "Reified.Refinements"
              Assembly.Load "Reified.Schema"
              Assembly.Load "Reified.Parse"
              Assembly.Load "Reified.Data" ]

        let removedTypes =
            [ "Reified.Validation.Validation`2"
              "Reified.Validation.Diagnostics`1"
              "Reified.Validation.PathSegment"
              "Reified.Validation.ValidateBuilder" ]
    ...
    let ``leaf packages stay independent of each other`` () =
        // Result and Constraint are independent leaves. Refinements depends only on Constraint; Schema depends
        // directly on Constraint, Refinements, Data, and Parse, never on Result — a consumer using another
        // library's result { } must be able to take Schema without an ambiguous builder.
        let leafPackages = [ "Reified.Result"; "Reified.Constraint"; "Reified.Schema" ]

        let allowedReferences =
            [ "Reified.Schema", "Reified.Constraint" ]
        ...
        // Schema owns no Result dependency: it consumes Constraint and Refinements directly.
        referencedAssemblyNames (Assembly.Load "Reified.Schema")
        |> assertContainsNone [ "Reified.Result" ]

        // Refinements depends on Constraint, never on Result.
        referencedAssemblyNames (Assembly.Load "Reified.Refinements")
        |> assertContainsAll [ "Reified.Constraint" ]

        referencedAssemblyNames (Assembly.Load "Reified.Refinements")
        |> assertContainsNone [ "Reified.Result" ]

    [<Fact>]
    let ``the umbrella carries no assembly and Reified_ErrorHandling stays retired`` () =
        // Reified is a dependency-only umbrella: installing it must not put a Reified.dll on the compile line,
        // because a type that could come from two assemblies is exactly what the umbrella exists to avoid.
        // Reified.ErrorHandling is retired outright — a grouping that is not a capability gets no package.
        let packages =
            [ "Reified.Result"
              "Reified.Constraint"
              "Reified.Refinements"
        ...
        for package in packages do
            referencedAssemblyNames (Assembly.Load package)
            |> assertContainsNone [ "Reified"; "Reified.ErrorHandling" ]

        for assemblyName in [ "Reified"; "Reified.ErrorHandling" ] do
            let probe = Path.Combine(AppContext.BaseDirectory, assemblyName + ".dll")
    ...
    let ``schema validation interpreters live alongside schema in the consolidated schema package`` () =
        ...
        test <@ schemaAssembly.GetName().Name = "Reified.Schema" @>

        schemaReferences
        |> assertContainsAll [ "Reified.Constraint"; "Reified.Refinements" ]
    ...
    [<Fact>]
    let ``schema types keep their shape and their assembly`` () =
        ...
        // The constraint read model belongs to Reified.Constraint, not to Schema: one declaration, one vocabulary.
        ...
        test <@ schemaAssembly.GetName().Name = "Reified.Schema" @>
        // Nothing in this repository may reach for the effect system.
        references
        |> assertContainsNone [ "Axial.Flow"; "Reified.Flow" ]
    ...
    let ``the constraint package publishes one value-rule vocabulary and no Check surface`` () =
        ...
        test <@ constraintAssembly.GetName().Name = "Reified.Constraint" @>
        ...
        let resultMembers =
            moduleTypeFromAssembly "Reified.Result" "Reified.Result.Result"
            |> publicStaticMemberNames
        ...
        test <@ typeof<Refinement<string, NonBlankString>>.Assembly.GetName().Name = "Reified.Refinements" @>
        assertModuleAbsentFromAssembly "Reified.Result" "Reified.Result.Parse"
        assertModuleAbsentFromAssembly "Reified.Refinements" "Reified.Refinements.Parse"
        test <@ not (referencedAssemblyNames (Assembly.Load "Reified.Refinements") |> Set.contains "Reified.Parse") @>
        ...
        let refineMembers =
            moduleTypeFromAssembly "Reified.Refinements" "Reified.Refinements.Refine"
            |> publicStaticMemberNames
        ...
        moduleTypeFromAssembly "Reified.Refinements" "Reified.Refinements.Refinement"
        |> publicStaticMemberNames
    ...
    [<Fact>]
    let ``the host-neutral HTTP boundary keeps its shape and stays host-free`` () =
        // Reified.Schema.Http assembles boundary input, problem details, and OpenAPI documents as values.
        // The moment it references a web framework or the effect system, it has stopped being host-neutral
        // and the split has leaked.
        let httpAssembly = Assembly.Load "Reified.Schema.Http"

        moduleTypeFromAssembly "Reified.Schema.Http" "Reified.Schema.Http.BoundaryInput"
        |> publicStaticMemberNames
        |> assertContainsAll [ "ofQuery"; "ofForm" ]

        // The record type ProblemDetails shadows the module of the same name, so name the module explicitly:
        // resolving the bare name would silently assert against the record and find none of these members.
        let problemDetailsModule =
            moduleTypeFromAssembly "Reified.Schema.Http" "Reified.Schema.Http.ProblemDetailsModule"

        problemDetailsModule
        |> publicStaticMemberNames
        |> assertContainsAll [ "ofErrors"; "ofParsed"; "malformedJson" ]

        // ContentType is a [<Literal>], so it is a static field rather than a property and does not appear in
        // publicStaticMemberNames. Its value is the wire contract, so assert the value and not just the name.
        let contentType =
            problemDetailsModule.GetField("ContentType", BindingFlags.Static ||| BindingFlags.Public)

        test <@ not (isNull contentType) @>
        test <@ contentType.GetRawConstantValue() = box "application/problem+json" @>

        moduleTypeFromAssembly "Reified.Schema.Http" "Reified.Schema.Http.Endpoint"
        |> publicStaticMemberNames
        |> assertContainsAll
            [ "get"; "post"; "put"; "patch"; "delete"
              "summary"; "operationId"; "tag"
              "accepts"; "returns"; "returnsJson"; "returnsProblemDetails" ]

        moduleTypeFromAssembly "Reified.Schema.Http" "Reified.Schema.Http.OpenApi"
        |> publicStaticMemberNames
        |> assertContainsAll [ "info"; "document"; "writeTo" ]

        assertTypePresentInAssembly "Reified.Schema.Http" "Reified.Schema.Http.EndpointSpec" |> ignore
        assertTypePresentInAssembly "Reified.Schema.Http" "Reified.Schema.Http.ProblemDetails" |> ignore
        assertTypePresentInAssembly "Reified.Schema.Http" "Reified.Schema.Http.OpenApiInfo" |> ignore

        let references = referencedAssemblyNames httpAssembly

        references |> assertContainsAll [ "Reified.Schema" ]
