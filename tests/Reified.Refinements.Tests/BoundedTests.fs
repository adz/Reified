...
module BoundedTests =
    ...
    let ``a bounded value can be admitted through its refinement`` () =
        ...
        test <@ Reified.Refinements.Refinement.create refinement 42 |> Result.map Bounded.value = Ok 42 @>
        test <@ Reified.Refinements.Refinement.create refinement 150 |> Result.isError @>
