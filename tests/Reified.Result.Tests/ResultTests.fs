...
module ResultTests =
    ...
    let ``Result covers fail-fast helpers and the result computation expression`` () =
        ...
        test <@ (42 |> Result.okIf (fun value -> value > 0)) = Ok 42 @>
        test <@ (-1 |> Result.okIf (fun value -> value > 0)) = Error () @>
        test <@ (42 |> Result.failIf (fun value -> value < 0)) = Ok 42 @>
        test <@ (-1 |> Result.failIf (fun value -> value < 0)) = Error () @>
        test <@ Result.require true = Ok () @>
        test <@ Result.require false = Error () @>
        test <@ (Result.require false |> Result.orError "required") = Error "required" @>
        ...
        test <@ (Some 42 |> Result.fromOption) = Ok 42 @>
        test <@ (None |> Result.fromOption) = Error () @>
        test <@ (ValueSome 42 |> Result.fromValueOption) = Ok 42 @>
        test <@ (ValueNone |> Result.fromValueOption) = Error () @>
