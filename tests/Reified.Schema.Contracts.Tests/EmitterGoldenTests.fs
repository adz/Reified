...
/// The corpus under tests/Reified.Schema.Tests/contracts is the emitter's contract: the checked-in
/// .g.fs files compile into Reified.Schema.Tests and pass behavior tests there, and these tests prove
...
module EmitterGoldenTests =
    ...
    let private corpusDirectory () =
        Path.Combine(__SOURCE_DIRECTORY__, "..", "Reified.Schema.Tests", "contracts")
        |> Path.GetFullPath
...
"""
        ...
        test <@ emitted.Contains "field \"sku\" (fun (value: Order) -> value.Sku)" @>
        test <@ emitted.Contains "field \"quantity\" (fun (value: Order) -> value.Quantity)" @>
